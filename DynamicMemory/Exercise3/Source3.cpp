...
int main()
{
	...
	int* MainPointer = nullptr;
	MemoryLeak2(MainPointer);

	std::cout << " why each of those functions creates a memory leak?" << std::endl;
	std::cout << "R: Because you can't access the variables out of its scope, but the variables are still stored in memory heap. So it can't be freed" << std::endl;
